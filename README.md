...
2. 대상 프로젝트 루트에서 `npx fcm-web init ... --install`으로 FCM 파일을 생성합니다.
3. 생성된 `src/lib/fcm.ts` 또는 `src/lib/fcm.js`를 앱 코드에서 import 해 FCM 클라이언트를 만듭니다.
...
기본 사용은 `init` 명령으로 설정 파일과 VAPID 키를 넘겨 실행하면 됩니다.
...
실제로 파일을 쓰지 않고 생성 경로와 설치 계획만 확인하고 싶으면 `--dry-run`을 사용합니다.
...
구조화된 결과가 필요하면 `--json`을 사용할 수 있습니다.
...
2. 대상 프로젝트 루트에서 `npx fcm-web init --config ./firebase.web.json --vapid-key ... --target . --install`을 실행합니다.
